...
- Application configuration remains in environment variables (`.env.example` documents the contract). Local `yarn start` runs `yarn setup` to prepare the Compose database; the application itself never provisions infrastructure. Production uses `yarn start:prod`, which does not run Docker setup.
