...
- Use the user's corrected resume as the authoritative input for tailoring to a supplied job description. Accept pasted text or import a public HTTPS job URL into editable text; require privacy confirmation before model calls.
- Stream separate resume analysis and job-description analysis before generating wording suggestions and factual review. Show every step, its status and bounded retries; keep schemas internal. Analysis outputs are bounded structured summaries, never private reasoning.
- Keep job selection/input, streamed analysis, suggestions and the updated resume on separate screens under `/tailoring`, with linked subroutes and previous-version history.
- Show each suggestion's original and proposed wording with an individual selection. Applying only the selected suggestions must preserve unselected wording and all factual fields. Preserve the original proposal so selections can be revisited; reapplying explicitly replaces the variant draft and requires approval again.
- Present the resulting resume on its own screen with inline review, draft saving, explicit approval and approved-version PDF/DOCX export. Direct links and reloads must restore persisted state.
