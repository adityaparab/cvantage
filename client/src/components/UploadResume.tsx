...
export default function UploadResume({
  ...
  useEffect(() => {
    api<Job[]>('/parsing-jobs')
      .then(setJobs)
      .catch(() => setError('Could not load in-progress uploads.'))
  }, [])
  const closeReview = useCallback(() => {
    setJob(null)
    api<Job[]>('/parsing-jobs')
      .then(setJobs)
      .catch(() => setError('Could not refresh uploads.'))
  }, [])
  async function upload(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setJob({ ...result, _id: result.jobId })
  ...
  async function open(id: string) {
    try {
      const result = await api<Job>(`/parsing-jobs/${id}`)
      setJob(result)
      setSource(result.source)
    } catch {
      setError('This upload is no longer available. Upload the original again.')
    }
  }
  async function prepare(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      setJob(
        await api<Job>(`/parsing-jobs/${job._id}/prepare`, {
          method: 'POST',
          body: JSON.stringify({
            source,
  ...
  return (
    <section className="panel upload-panel">
      ...
      {!job ? (
        <>
          <p className="muted">
            Add your contact details as written on your resume so we can keep
            them separate while preparing your application.
          </p>
          <form onSubmit={(event) => void upload(event)}>
            ...
            <p className="hint">
              PDF, DOCX, or DOC · up to 20 MB. The original file is discarded
              after extraction.
            </p>
        ...
        <form onSubmit={(event) => void prepare(event)}>
          <h3>Review before AI processing</h3>
          <p className="muted">
            Check that your name, phone, email, and location are removed
            everywhere. Edit anything we missed. Your contact details are stored
            separately for your final download.
          </p>
          <label>
