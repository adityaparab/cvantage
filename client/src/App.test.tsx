...
it('signs in and loads the private resume workspace', async () => {
  const fetcher=vi.fn().mockResolvedValueOnce(response({message:'Sign in'},401)).mockResolvedValueOnce(response({id:'owner',email:'test@example.test',csrfToken:'token'})).mockResolvedValueOnce(response([])).mockResolvedValueOnce(response([]))
...
it('shows server errors without entering the workspace', async () => {
  vi.stubGlobal('fetch',vi.fn().mockResolvedValueOnce(response({},401)).mockResolvedValueOnce(response({message:'Invalid email or password'},401)))
  render(<App />)
  fireEvent.change(await screen.findByLabelText('Email'),{target:{value:'test@example.test'}})
  fireEvent.change(screen.getByLabelText('Password'),{target:{value:'synthetic-password-123'}})
  fireEvent.click(screen.getByRole('button',{name:'Sign in'}))
