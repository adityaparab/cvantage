...
it('signs in and loads the private resume workspace', async () => {
  const fetcher=vi.fn().mockResolvedValueOnce(response({message:'Sign in'},401)).mockResolvedValueOnce(response({id:'owner',email:'test@example.test',csrfToken:'token'})).mockResolvedValueOnce(response([]))
